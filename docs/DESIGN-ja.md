...
部分日付を拒否する理由: `+/-HH` 形式の短い TZ オフセット（`+09`, `-5` 等）と部分日付の末尾（`-01`, `-9`）が構文的に曖昧になるため。YMD すべて必須とすることで曖昧さを排除する（→ DR-0009）。
...
詳細は [`docs/decisions/`](./decisions/INDEX.md) を参照:
- DR-0003: カスタム epoch パラメータ
- DR-0005: TzOffset の範囲バリデーションと pub(all) 維持
- DR-0006: 設計決定まとめ（parse_range 方式、ago、EpochTime 命名、FFI 等）
- DR-0007: TimeSpec マルチパスパーサ
- DR-0008: 追加の設計決定（@冪等性、Eq 方針、部分日付、Local デフォルト、raw epoch）
- DR-0009: 部分日付の禁止と detect_tz_suffix の新設
